...
export class MemStorage implements IStorage {
  ...
  async getUserBySession(sessionId: string): Promise<User | undefined> {
    // 세션 스토어는 글로벌에 있으므로 직접 접근
    if (global.warehouseSessions && global.warehouseSessions.has(sessionId)) {
      const sessionData = global.warehouseSessions.get(sessionId);
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllUsers(): Promise<User[]> {
    return await this.db.select().from(users).orderBy(asc(users.createdAt));
  }
...
// 간단한 저장소 초기화 - 데이터베이스 연결 문제시 MemStorage 사용
console.log("DATABASE_URL:", process.env.DATABASE_URL ? "exists" : "not found");

// 현재 DATABASE_URL 연결 실패 문제로 인해 임시로 MemStorage 사용
